...
namespace Terraria.ModLoader.Audio
{
	public static class XWBFixer
	{
		...
		private static void ParseWaveBank(string input, string output)
		{
			...
			{
				String debugPath = typeof(Main).Assembly.Location;
				...
				AddSoundToSoundTrack(wavebank, reader, curOffset, header, playRegionOffset, tracks);
			}
		}

		private static void AddSoundToSoundTrack(WaveBankInfo wavebank, BinaryReader reader, uint curOffset, HeaderInfo header,
			uint playRegionOffset, SoundEntry[] tracks)
		{
			for (int curEntry = 0; curEntry < wavebank.numEntries; curEntry += 1)
			{
				...
				if (wavebank.metadataElementSize >= 4)
				{
					entry.FlagsAndDuration = reader.ReadUInt32();
				}

				if (wavebank.metadataElementSize >= 8)
				{
					entry.Format = reader.ReadUInt32();
				}

				if (wavebank.metadataElementSize >= 12)
				{
					entry.PlayOffset = reader.ReadUInt32();
				}

				if (wavebank.metadataElementSize >= 16)
				{
					entry.PlayLength = reader.ReadUInt32();
				}

				if (wavebank.metadataElementSize >= 20)
				{
					entry.LoopOffset = reader.ReadUInt32();
				}

				if (wavebank.metadataElementSize >= 24)
				{
					entry.LoopLength = reader.ReadUInt32();
				}
				else
				{
